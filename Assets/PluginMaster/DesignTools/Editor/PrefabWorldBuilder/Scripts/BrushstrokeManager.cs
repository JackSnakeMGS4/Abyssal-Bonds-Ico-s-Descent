        ...
        public static void UpdateShapeBrushstroke()
        {
            ...
            void AddItemsToLine(Vector3 start, Vector3 end, ref int nextIdx)
            {
                ...
                do
                {
                    ...
                    var scale = Scale(nextItem);
                    float itemSize;
                    var key = (nextIdx, scale);
                    if (nextItem.randomScaleMultiplier) itemSize = GetLineSpacing(nextIdx, settings, scale);
                ...
                foreach (var item in items)
                {
                    ...
                    var nextItem = PaletteManager.selectedBrush.items[item.idx];
                    var scale = Scale(nextItem);
            ...
            if (settings.shapeType == ShapeSettings.ShapeType.CIRCLE)
            {
                ...
                do
                {
                    ...
                    else if (PaletteManager.selectedBrush != null)
                    {
                        ...
                        var scale = Scale(nextItem);
                        var key = (nextItemIdx, scale);
                        if (nextItem.randomScaleMultiplier) itemSize = GetLineSpacing(nextItemIdx, settings, scale);
                        else if (prefabSpacingDictionary.ContainsKey(key)) itemSize = prefabSpacingDictionary[key];
                        else
                        {
