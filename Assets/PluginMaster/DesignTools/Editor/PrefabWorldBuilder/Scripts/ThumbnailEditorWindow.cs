...
namespace PluginMaster
{
    ...
    public class ThumbnailEditorCommon : ThumbnailEditorWindow
    {
        ...
        protected override void Initialize(BrushSettings brush)
        {
            ...
            foreach (var item in brushItems)
            {
                ...
                ThumbnailUtils.UpdateThumbnail(subThumbnail.overwrite ? subThumbnail.settings : _settings,
                    subThumbnail.texture, subThumbnail.prefab, subThumbnail.multibrushItem.thumbnailPath);
            }
            var included = GetIncluded();
            ThumbnailUtils.UpdateThumbnail(_settings, _thumbnail, included, brushSettings.thumbnailPath);
        }
        ...
        protected override void OnApply()
        {
            ...
            ThumbnailUtils.UpdateThumbnail(_brush as MultibrushSettings, true);
        ...
        protected override void OnSettingsChange()
        {
            ...
            var included = GetIncluded();
            ThumbnailUtils.UpdateThumbnail(_settings, _thumbnail, included, _brush.thumbnailPath);
        }
    ...
    public class SubThumbnailEditor : ThumbnailEditorWindow
    {
            ...
            using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
            {
                using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (check.changed)
                    {
                        ...
                        ThumbnailUtils.UpdateThumbnail(_settings, _thumbnail, _itemClone.prefab, null);
                    }
                }
            }
        ...
        protected override void Initialize(BrushSettings brush)
        {
            ...
            base.Initialize(brush);
            ThumbnailUtils.UpdateThumbnail(_settings, _thumbnail, item.prefab, item.thumbnailPath);
        }
        protected override void OnApply()
        {
            UnityEditor.Undo.RegisterCompleteObjectUndo(this, UNDO_CMD);
            ...
            ThumbnailUtils.UpdateThumbnail(item.parentSettings, false);
            PaletteManager.selectedPalette.Save();
        }
        ...
        protected override void OnSettingsChange()
        {
            ...
            ThumbnailUtils.UpdateThumbnail(_settings, _thumbnail, _itemClone.prefab, _itemClone.thumbnailPath);
        }
